...
add_heading_1("8.1 Cấu Hình Môi Trường Thực Thi & Cấu Trúc Mã Nguồn")
add_p("Hệ thống LMS AI được xây dựng theo kiến trúc tách biệt giữa tầng Giao diện (Next.js 14 App Router) và tầng Xử lý nghiệp vụ (NestJS 12 Framework). Dưới đây là các cấu hình môi trường cần thiết để triển khai thực tế:")

add_heading_2("8.1.1 Cấu hình Biến Môi trường (.env)")
add_p("Toàn bộ các thông số kết nối cơ sở dữ liệu, khóa bảo mật JWT và API Key dịch vụ AI được cô lập trong file .env phía máy chủ để đảm bảo an toàn thông tin:")
add_bullet("DATABASE_URL: Chuỗi kết nối bảo mật SSL tới hệ quản trị CSDL Neon Serverless PostgreSQL qua kênh Pooling tự động.", "• ")
add_bullet("JWT_SECRET & JWT_EXPIRES_IN: Khóa bí mật dùng để ký chữ ký số HMAC-SHA256 cho mã Token xác thực của 4 vai trò (thời hạn 24 giờ).", "• ")
add_bullet("GEMINI_API_KEY: Khóa truy cập Google AI Studio để gọi mô hình ngôn ngữ lớn Google Gemini.", "• ")

add_heading_2("8.1.2 Cấu trúc Tổ chức Mã nguồn Dự án")
add_p("Mã nguồn hệ thống được phân rã khoa học thành các thư mục chức năng chuyên biệt:")
add_bullet("backend/: Chứa toàn bộ mã nguồn xử lý nghiệp vụ Backend xây dựng bằng NestJS gồm 8 module (auth, users, courses, classes, enrollments, attendances, grades, stats, ai) cùng thư mục prisma/ quản lý Database Schema 3NF.", "• ")
add_bullet("frontend/: Chứa 23 màn hình giao diện người dùng Next.js App Router (thư mục app/admin, app/teacher, app/student, app/staff), tích hợp TailwindCSS và hệ thống định tuyến RBAC.", "• ")
add_bullet("docs/: Chứa toàn bộ tài liệu thiết kế hệ thống, sơ đồ kỹ thuật UML/ERD và cẩm nang hướng dẫn vận hành.", "• ")

add_heading_1("8.2 Khởi Tạo Cơ Sở Dữ Liệu & Nạp Dữ Liệu Mẫu (Database Seeding)")
...
add_p("Quy trình khởi tạo CSDL được thực hiện hoàn toàn tự động qua công cụ Prisma ORM:")
add_bullet("Đồng bộ Schema: Thực thi lệnh npx prisma db push để khởi tạo 14 bảng quan hệ chuẩn 3NF, thiết lập đầy đủ khóa chính (PK), khóa ngoại (FK) và các ràng buộc toàn vẹn trên Neon Cloud.", "Bước 1: ")
add_bullet("Nạp dữ liệu mẫu: Thực thi lệnh npm run db:seed để nạp sẵn danh mục 6 tài khoản người dùng cho 4 vai trò (admin01, teacher01, teacher02, staff01, student01, student02), các khóa học và lớp học ban đầu với mật khẩu băm Argon2 chuẩn an toàn.", "Bước 2: ")

add_heading_1("8.3 Kết Quả Cài Đặt Giao Diện & Các Phân Hệ Chức Năng Chính")
add_p("Dưới đây là kết quả cài đặt và giao diện thực tế tiêu biểu của 6 phân hệ cốt lõi trong hệ thống:")

add_heading_2("8.3.1 Phân Hệ 1: Đăng Nhập & Bàn Làm Việc Quản Trị Trung Tâm")
add_p("Màn hình đăng nhập (SCR-AUTH-01) tích hợp sẵn các nút chọn nhanh tài khoản mẫu cho 4 vai trò, tự động phân giải vai trò qua JWT và chuyển hướng chính xác về Dashboard Quản trị (SCR-ADM-01) với các chỉ số hoạt động tổng quan:")
...
# Save unified docx
output_file1 = r"D:\MyProjects\lms-ai\docs\design\CHUONG_8_9_BAO_CAO_HOAN_CHINH.docx"
output_file2 = r"D:\MyProjects\lms-ai\docs\design\CHUONG_8_9_CAI_DAT_TRIEN_KHAI_KIEM_THU_HUONG_DAN_SD.docx"
doc.save(output_file1)
doc.save(output_file2)
print("SUCCESS: Updated both docx files perfectly!")
