# Test Report — ETC English Center LMS AI
...
## Kết quả kiểm thử toàn diện (45 ca kiểm thử)
...
**Đánh giá chất lượng tổng thể:** 45/45 ca kiểm thử đạt trạng thái **PASS** (Tỷ lệ thành công **100%**).

Hệ thống đáp ứng đầy đủ tất cả các quy tắc nghiệp vụ, kiểm soát chặt chẽ các trường hợp biên và sẵn sàng đưa vào vận hành thực tế.

### Điểm nổi bật đã xác nhận:

| Hạng mục | Kết quả |
| :--- | :--- |
| Xác thực & phân quyền JWT/Argon2 (4 vai trò) | Hoạt động chính xác |
| ACID Transaction đăng ký lớp + tự sinh hóa đơn | Hoạt động chính xác |
| Khống chế sĩ số tối đa 25 học viên/lớp | Hoạt động chính xác |
| Chống trùng lịch phòng học & giáo viên | Hoạt động chính xác |
| Chặn đăng ký sai CEFR / trùng lịch học viên | Hoạt động chính xác |
| Công thức điểm TK = CC×20% + GK×30% + CK×50% | Cho kết quả 84.50 — Chính xác |
| Xét ĐẠT: TK >= 50 và CC >= 80% | Hoạt động chính xác |
| AI Tư vấn lớp — Fallback khi mất mạng | Hoạt động chính xác |
| AI Sinh đề — Fallback khi timeout 10s | Hoạt động chính xác |
| AI Audit Log — Ghi nhật ký mọi lượt gọi AI | Hoạt động chính xác |

### Bug Report

> Không có bug nào được ghi nhận. Tất cả 45 ca kiểm thử đều PASS.
